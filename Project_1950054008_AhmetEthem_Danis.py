...
class NeuralNetwork: # Yapay sinir ağı
    ...
    def predict(self):
        ...
        return self.layer3 # 0-1 arasında bir değer
...
class Bird: # kuş
    def __init__(self,Bird_Image, Bird_Mask, weights1 = np.random.rand(7,3) - 0.5,weights2 = np.random.rand(1,7) - 0.5,
        ...
        self.Gravity = 0.75
        self.acc = 0.075
        self.Bird_Image = Bird_Image # Kuş resmi
        self.Bird_Mask = Bird_Mask # Kuş maskesi
        ...
        # Kuşun sinir ağının hazırlanması
        ...
        # kuşun ekrana çizdirilmesi
    def Draw_Bird(self,window):
        window.blit(self.Bird_Image,(int(self.Bird_X), int(self.Bird_Y)))

        # Sinir ağının verilerinin güncellenmesi
    def Update_NN(self):
        self.Bird_Network.X = self.Bird_Y # Kuşun yükseliği
        self.Bird_Network.Y = self.Bird_Distance_With_Pipe # Kuş ile boru arasındaki mesafe
        self.Bird_Network.Z = self.Pipe_Height # Borunun yüksekliği
    ...
    def Bird_Jump(self):
        if self.Bird_Network.predict() < 0.5: # 0.5 altı gelirse zıpla
            self.Gravity = -2
...
class GameCore:
    def __init__(self, Population_Number = 1):
        ...
        self.Population = [] # Popülasyon
        self.Next_Generation = [] # Yaratılan Sonraki Nesli barındıran liste
        self.Population_Number = Population_Number
        self.Died_Bird = [] # Popülasyonda ölen kuşların listesi
        self.Generation_Timer = 0

        # en ilk random üretilen nesil # başlangıç popülasyonu
    ...
    def Draw(self):
        ...
        self.window.blit(self.BackGround,(0,0)) # ekrana arka planı çizdir
        self.window.blit(self.Base,(0, 400))

        for pipe in self.Pipe_List: # borular listesindeki her borunun çizim fonksiyonunu çağır
            pipe.Draw_Pipe(self.window)

        for i in self.Population: # Popülasyon listesindeki her kuşun çizim fonksiyonunu çağır
            i.Draw_Bird(self.window)

        # Ekrana yazı yazdırma # jenarasyon sayısı
        self.window.blit(self.Font_T.render("" + str(self.Generation_Timer), True, (255, 255, 255)), (20, 20))

        self.Clock.tick(60) # 60fpse sabitleme
        pygame.display.update() # ekranı döngü sonunda güncelleme

    def create_weights(self): # başlangıç popülasyonunun rastgele ağırlıklarını üretmek için
        weights1 = np.random.rand(7, 3) - 0.5 # -0.5 aykırılık için
    ...
    def Crossover(self):
        # objelerin belilri değerlerine göre küçükten büyüğe sıralar
        ...
        if self.Died_Bird[-1].Score == 0: # or self.Died_Bird[-1].Score == 1:
            ...
            last_best = int((98 * self.Population_Number) / 100) # %98. nokta
            self.Next_Generation = []
            self.Next_Generation.extend(self.Population[last_best:]) # o noktan sonrasını al
            ...
            while True:
                if len(self.Next_Generation) < self.Population_Number:
                    ...
                    for x,y in zip(member_1_weight_1, member_2_weight_1):
                        for i,k in zip(x,y):
                            Prob = random.random() #0-1 arasında random bir değer
                            if Prob < 0.47:
                                chield_1_weight_1.append(i) # üye birin ağırlığını
                            elif Prob < 0.94:
            ...
            self.Population = self.Next_Generation # sonraki nesil popülasyon haline geliyor
        ...
        for pipe in self.Pipe_List:
            ...
            for Member in self.Population:
                ...
                if collision_lower != None or collision_upper != None:
                    self.Died_Bird.append(Member) # ölü kuş listesine ekle
                    self.Population.remove(Member) # eklediğin kuşu popülasyondan sil

                if Member.Bird_X + 16 == pipe.Pipe_X:
                    Member.Score += 1

        for Member in self.Population:
            if Member.Bird_Loop() == "Died": # yere çarpma durumu ve yerçekimi belirler
